...
plot_info <- read.csv("In_Plot_info.csv", stringsAsFactors = FALSE) %>%
  ...
  mutate(Ward = factor(Ward, levels = Ward_faclev), 
    Dist_level = factor(Dist_level, levels = Dist_level_faclev), 
...
# of trees
scatterplotMatrix(subset(tree_diversity, select = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness")))
cor(subset(tree_diversity, select = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness")), use = "pairwise.complete.obs") %>% 
  as.data.frame() %>%
  mutate(rownames = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness")) %>%
  pivot_longer(cols = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness"), names_to = "colnames", values_to = "corindex") %>% 
  mutate(rownames = factor(rownames, levels = c("Evenness", "Simpson", "Shannon", "Richness", "Sum_stem")), 
         colnames = factor(colnames, levels = c("Evenness", "Simpson", "Shannon", "Richness", "Sum_stem"))) %>%
  ggplot(aes(rownames, colnames, fill = corindex)) + geom_tile() + 
  scale_fill_gradient2(limits = c(-1,1), high = "blue", low = "red", midpoint = 0) + 
  geom_text(aes(label = round(corindex, digits = 3)), size = 5) + 
  scale_x_discrete(limits = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness"), expand = c(0, 0)) + 
  scale_y_discrete(expand = c(0, 0))

# cor indexes of shrubs
scatterplotMatrix(subset(shrub_diversity, select = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness")))
cor(subset(shrub_diversity, select = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness")), use = "pairwise.complete.obs") %>% 
  as.data.frame() %>%
  mutate(rownames = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness")) %>%
  pivot_longer(cols = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness"), names_to = "colnames", values_to = "corindex") %>% 
  mutate(rownames = factor(rownames, levels = c("Evenness", "Simpson", "Shannon", "Richness", "Sum_area")), 
         colnames = factor(colnames, levels = c("Evenness", "Simpson", "Shannon", "Richness", "Sum_area"))) %>%
  ggplot(aes(rownames, colnames, fill = corindex)) + geom_tile() + 
  scale_fill_gradient2(limits = c(-1,1), high = "blue", low = "red", midpoint = 0) + 
  geom_text(aes(label = round(corindex, digits = 3)), size = 5) + 
  scale_x_discrete(limits = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness"), expand = c(0, 0)) + 
  scale_y_discrete(expand = c(0, 0))

# kruskal test & boxplot for trees
{
  pvalue_list <- vector("list", 3)
  for (i in c("Sum_stem", "Richness", "Shannon","Simpson", "Evenness")) {
...
tree_diversity %>% subset(select = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness", 
  ...
  pivot_longer(cols = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness"), 
  ...
  mutate(index = factor(index, levels = c("Sum_stem", "Richness", "Shannon", "Simpson", "Evenness")), 
...
{
  pvalue_list <- vector("list", 3)
  for (i in c("Sum_area", "Richness", "Shannon","Simpson", "Evenness")) {
    for (j in c("Ward", "Landuse_class", "Plot_pub_pri_level")) {
      pvalue_list[[1]] <- c(pvalue_list[[1]], i)
      pvalue_list[[2]] <- c(pvalue_list[[2]], j)
...
shrub_diversity %>% subset(select = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness", 
                                      "Ward", "Landuse_class", "Plot_pub_pri_level")) %>% 
  pivot_longer(cols = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness"), 
               names_to = "index", values_to = "index_value") %>% 
  pivot_longer(cols = c("Ward", "Landuse_class", "Plot_pub_pri_level"), 
               names_to = "attr", values_to = "attr_value") %>% 
  mutate(index = factor(index, levels = c("Sum_area", "Richness", "Shannon", "Simpson", "Evenness")), 
         attr = factor(attr, levels = c("Ward", "Landuse_class", "Plot_pub_pri_level")), 
         attr_value = factor(attr_value, levels = c(Ward_faclev, Landuse_class_faclev, Plot_pub_pri_level_faclev))) %>% 
  na.omit() %>%
  ggplot(aes(attr_value, index_value)) + geom_boxplot() + 
