...
Plot_pub_pri_level_faclev <- c("Private area", "Mix area", "Public area")
...
for (i in c("Ward", "Dist_level", "Landuse_class", "Plot_pub_pri_level")) {
  x <- anosim(as.data.frame(mds_plot_selected)[,c(1:142)], as.data.frame(mds_plot_selected)[,i], 
...
# for (i in c("Ward", "Dist_level", "Landuse_class", "Plot_pub_pri_level")) {
#   x <- combn(unique(as.data.frame(mds_plot_selected)[,i]), 2)
#   for (j in 1:ncol(x)) {
#     y <- mds_plot_selected %>% as.data.frame() %>% 
#       filter(mds_plot_selected[[i]] == x[1,j] | mds_plot_selected[[i]] == x[2,j]) 
#     z <- anosim(y[1:142], y[,i])
#     if (z$signif < 0.01) cat(as.character(x[,j]), z$signif, "\n", sep = "  ")
#   }
# }
# rm(x, y, z)

for (i in c("Ward", "Dist_level", "Landuse_class", "Plot_pub_pri_level")) {
  a <- combn(unique(as.data.frame(mds_plot_selected)[,i]), 2)
  z <- vector("list", 3)
  for (j in 1:ncol(a)) {
    b <- mds_plot_selected %>% as.data.frame() %>% 
      filter(mds_plot_selected[[i]] == a[1,j] | mds_plot_selected[[i]] == a[2,j]) 
    c <- anosim(b[1:142], b[,i])
    z[[1]] <- c(z[[1]], as.character(a[1,j]))
    z[[2]] <- c(z[[2]], as.character(a[2,j]))
    z[[3]] <- c(z[[3]], c$signif)
    ### should make a new data frame and use ggplot to generate heatmap
  }
}
rm(x, y, z)
