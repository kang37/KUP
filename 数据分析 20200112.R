...
# 聚合到一张图上，但是没有标签
# 乔木
layout(mat = matrix(1:20, ncol = 5))
for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
  for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
    In1 <- pairwise.wilcox.test(Tree.comm[,i], Tree.comm[,j], p.adjust.method = "BH", exact=FALSE)
    # 注意需要反转一下矩阵，因为图是反着的
    In2 <- t(apply(In1$p.value, 2, rev))
    image(In2, col = c("red", "orange", "yellow", "grey"), breaks = c(0, 0.001, 0.01, 0.05, 1), 
          xlab = NA, ylab = NA, axes=FALSE, main = paste(i, "~", j, sep = ""))
  }
}
# 灌木
layout(mat = matrix(1:20, ncol = 5))
for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
  for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
    In1 <- pairwise.wilcox.test(Shrub.comm[,i], Shrub.comm[,j], p.adjust.method = "BH", exact=FALSE)
    # 注意需要反转一下矩阵，因为图是反着的
    In2 <- t(apply(In1$p.value, 2, rev))
    image(In2, col = c("red", "orange", "yellow", "grey"), breaks = c(0, 0.001, 0.01, 0.05, 1), 
          xlab = NA, ylab = NA, axes=FALSE, main = paste(i, "~", j, sep = ""))
...
# 作箱线图
# 乔木
layout.show(layout(mat = matrix(1:20, nrow = 5),widths = c(11, 4, 6, 2)))
for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
  for (i in c("Species_sum","Richness", "Shannon","Simpson", "Evenness")) {
    boxplot(Tree.comm[,i]~Tree.comm[,j], ylab = i, xlab = j, par(las=2))
  }
}
# 灌木
layout.show(layout(mat = matrix(1:20, nrow = 5),widths = c(11, 4, 6, 2)))
for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
  for (i in c("Species_sum","Richness", "Shannon","Simpson", "Evenness")) {
    boxplot(Shrub.comm[,i]~Shrub.comm[,j], ylab = NA, xlab = NA, par(las=2))
  }
}
