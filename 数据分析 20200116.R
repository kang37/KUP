...
{
  # 定义会出现的因子水平
  F_level_ward <- c("Kamigyō-ku","Shimogyō-ku", "Sakyō-ku","Nakagyō-ku","Higashiyama-ku", "Ukyō-ku","Fushimi-ku", "Kita-ku","Minami-ku", "Yamashina-ku","Nishikyō-ku")
  # 读取数据
  In.plantinfo <- read.csv("In_plant_info.csv", stringsAsFactors = FALSE)
  In.plotinfo <- read.csv("In_plot_info.csv", stringsAsFactors = FALSE) %>%
    mutate(Ward = factor(Ward, levels = F_level_ward))
  In.tree.data <- read.csv("In_tree_data.csv", stringsAsFactors = FALSE) %>% 
    subset(Plot_ID!=67) %>% # 删除异常点 
    left_join(In.plantinfo, by = "Species_CN")
  ...
  ## 乔木
  # 将输入数据变成物种-数量矩阵
  Tree.comm <- In.tree.data[,c("Plot_ID", "Species_CN")] %>%
    mutate(value=1) %>%
    pivot_wider(names_from = Species_CN, values_from = value,
                values_fn = list(value=sum), values_fill = list(value=0)) %>%
    as.data.frame() 
  
  Tree.comm2$Species_sum <- rowSums(Tree.comm2[2:143])
...
# 构造乔木小图注释p值矩阵
{
  ...
  for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
    for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
      ...
      pvalue_list[[3]] <- c(pvalue_list[[3]], round(kruskal.test(Tree.comm[, i] ~ Tree.comm[, j])$p.value,3))
    }
  }
}
...
# 做乔木箱线图
Tree.comm[,c("Plot_ID", "Species_sum","Richness", "Shannon","Simpson", "Evenness", 
             "Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")] %>%
  pivot_longer(cols = c("Species_sum","Richness", "Shannon","Simpson", "Evenness"), 
               names_to = "index", values_to = "index_value", values_drop_na = TRUE) %>%
  pivot_longer(cols = c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new"),
               names_to = "attr", values_to = "attr_value", values_drop_na = TRUE) %>%
  mutate(index = factor(index,levels = c("Species_sum","Richness", "Shannon","Simpson", "Evenness")),
         attr = factor(attr, levels = c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")), 
         attr_value = factor(attr_value, levels = c(
           "Kamigyō-ku","Shimogyō-ku", "Sakyō-ku","Nakagyō-ku","Higashiyama-ku", "Ukyō-ku","Fushimi-ku", "Kita-ku","Minami-ku", "Yamashina-ku","Nishikyō-ku", 
           "Center","Cen-Mid","Mid-Sub","Suburban", 
           "Com","Com neigh","R low","R high","R resi","Ind", 
           "N","Y"))) %>%
  ggplot(aes(x=attr_value, y=index_value)) + geom_boxplot() +
...
# 构造灌木小图注释p值矩阵
{
  ...
  for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
    for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
      pvalue_list[[1]] <- c(pvalue_list[[1]], i)
      pvalue_list[[2]] <- c(pvalue_list[[2]], j)
      pvalue_list[[3]] <- c(pvalue_list[[3]], round(kruskal.test(Shrub.comm[, i] ~ Shrub.comm[, j])$p.value,3))
    }
  }
  }
...
# 做灌木箱线图
Shrub.comm[,c("Plot_ID", "Species_sum","Richness", "Shannon","Simpson", "Evenness", 
              "Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")] %>%
  pivot_longer(cols = c("Species_sum","Richness", "Shannon","Simpson", "Evenness"), 
               names_to = "index", values_to = "index_value", values_drop_na = TRUE) %>%
  pivot_longer(cols = c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new"),
...
# 查看各个具体的图和数据
# 乔木
layout.show(layout(mat = matrix(1:20, ncol = 4)))
for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
  for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
    In1 <- pairwise.wilcox.test(Tree.comm[,i], Tree.comm[,j], p.adjust.method = "BH", exact=FALSE)
    p <- pheatmap(In1$p.value, cluster_rows = FALSE, cluster_cols = FALSE, 
             color = c("red", "orange", "yellow", "grey"), breaks = c(0, 0.001, 0.01, 0.05, 1),
             display_numbers = TRUE, main = paste(i, "~", j))
  }
}
# 灌木
...
for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
  for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
    pair_result_1 <- dunn.test(Tree.comm[,i], Tree.comm[,j], altp = TRUE)
    pair_result_2 <- data.frame(comparison = pair_result_1$comparison, p=pair_result_1$altP.adjusted)
    pair_result_2 <- pair_result_2[which(pair_result_2$p<0.05), ]
    tree_pair <- c(tree_pair, pair_result_2)
  }
}
print(tree_pair)
...
for (i in c("Species_sum", "Richness", "Shannon","Simpson", "Evenness")) {
  for (j in c("Ward", "Dist_level", "Landuse_class", "Pub_Pri_new")) {
    pair_result <- vector("list",2)
    pair_result[[1]] <- dunn.test(Tree.comm[,i], Tree.comm[,j], altp = TRUE)$comparison
    pair_result[[2]] <- dunn.test(Tree.comm[,i], Tree.comm[,j], altp = TRUE)$altP.adjusted
    pair_result_dat <- data.frame("com"=pair_result[[1]], "p"=pair_result[[2]]) %>% 
      separate(com, c("com1", "com2"), " - ")
    pair_plot <- c(pair_plot, 
                   ggplot(pair_result_dat, aes(com1, com2)) + 
...
fac2 <- fct_reorder(fac, x, mean)
res2 <- tapply(x, fac2, mean)
barplot(res2)
